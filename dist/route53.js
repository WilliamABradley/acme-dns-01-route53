import { ChangeResourceRecordSetsCommand, GetChangeCommand, ListHostedZonesByNameCommand, ListResourceRecordSetsCommand, Route53Client, } from "@aws-sdk/client-route-53";
const getZones = async (client) => {
    ...
    return {
        ...
        set: async (data) => {
            ...
            try {
                ...
                const recordSetResults = await client.send(new ListResourceRecordSetsCommand({ HostedZoneId: zone.Id }));
                if (config.debug) {
                    console.log(`No existing records for ${recordName} in \n\t in:`, recordSetResults.ResourceRecordSets?.map((rrs) => {
                        return {
                            name: rrs.Name,
                ...
                if (existingRecord.length) {
                    ...
                    if (config.debug) {
                        console.log("\t setting it to:", resourceRecords.map((rrs) => rrs.Value).join(","));
                    }
                }
                else {
                    if (config.debug) {
                ...
                if (config.ensureSync) {
                    ...
                    while (status === "PENDING") {
                        const timeout = 5000;
                        if (config.debug) {
                            console.log(`\t but ... change is still pending. Will check again in ${timeout / 1000} seconds.`);
                        }
                        await sleep(timeout);
        ...
        remove: async (data) => {
            ...
            try {
                ...
                // find record first
                const data = await client.send(new ListResourceRecordSetsCommand({ HostedZoneId: zone.Id }));
                if (config.debug) {
                    console.log("\n\t from one of these existing records:", data.ResourceRecordSets?.map((rrs) => {
                        return {
                            name: rrs.Name,
                            value: rrs.ResourceRecords?.map((rrs) => rrs.Value).join(","),
                        };
                ...
                const match = data.ResourceRecordSets?.filter((rrs) => rrs.ResourceRecords?.filter((txtRs) => txtRs.Value?.slice(1, -1) === txt).length // remove quotes around record and match it against value we want to remove
                ...
                if (match && match.ResourceRecords?.length > 1) {
                    ...
                    // upsert
                    const rr = match.ResourceRecords?.filter((rr) => rr.Value?.slice(1, -1) !== txt // remove quotes
                    );
                    if (config.debug) {
                        console.log("\t new records will look like this:", rr.map((r) => r.Value));
                    }
        ...
        get: async (data) => {
            ...
            try {
                ...
                const txtRecords = data.ResourceRecordSets?.filter((rrs) => rrs.Type === "TXT");
                if (config.debug) {
                    console.log("\t existing txt values:", txtRecords);
                }
                const match = txtRecords
                    .map((rrs) => rrs.ResourceRecords?.map((rec) => rec.Value?.slice(1, -1)) // remove quotes sorrounding the strings
                )
                    .filter((txtRecords) => {
