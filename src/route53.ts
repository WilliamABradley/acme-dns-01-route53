...
type Config = Partial<Route53ClientConfig> & {
  debug?: boolean;
  ensureSync?: boolean;
};
  ...
  return {
    ...
    set: async (data: any) => {
      ...
      try {
        ...
        const recordSetResults = await client.send(
        ...
        const setResults = await client.send(
        ...
        if (config.ensureSync) {
          ...
          while (status === "PENDING") {
            ...
            if (config.debug) {
              console.log(
                `\t but ... change is still pending. Will check again in ${
                  timeout / 1000
                } seconds.`
              );
            }
